...
class Nodelet1(Node):
    def __init__(self):
        ...
        self.lift_leg_distance = 0.44   # 전체 길이 50cm - 다리 두께 1.5 *2 (다리 두께 절반이 1.5cm)
...
class Nodelet2(Node):
    def __init__(self):
        ...
        self.joy_auto_sub = self.create_subscription(String, '/joy_step', self.joy_auto_callback, 10)
        self.joy = True
        self.joy_step = False
        ...
        self.e3_old = 0
        self.e3_threshold = 0.5
        self.step = 0
        self.distance_center = 0.70
        self.distance_stop_center = 0.33
    ...
    def shutdown_node(self):
        self.get_logger().info('Shutting down lqr_node')
    ...
    def joy_auto_callback(self, data):
        ...
        if message == 'step_5':
            self.joy_step = True
            self.get_logger().info(f'{self.joy_step}')
    ...
    def timer_callback(self):
        ...
        if self.lifecycle == True:
            ...
            if self.step == 5 and self.e5_count > 500:
                self.step = 6
                self.step_1_1 = True
                self.joy_step = False
            ...
            if self.e3_count == 11 and self.step != 5:
                self.step = 1
                self.step_1_1 = True  # 뒷다리로 바뀌기 위한 첫번째 조건 (앞다리 인식 후)

            if self.step_1_1:
                if self.e3 < 0.4:    # 앞다리 가까워졌을 때 : 두번째 조건
                    self.step_1_2 = True
            ...
            # self.get_logger().info(f'{self.step_1_1},{self.step_1_2},{self.step_2_ready},{self.e3}')
            


            if self.ekf_slam is not None and self.step_2_ready and (self.step == 1 or self.step == 6):
                self.step = 2
            ...
            if self.ekf_slam is not None:
                ...
                # x_,y_ = self.transformation((0,0,-np.pi),(self.ekf_slam.x_hat[0][0],self.ekf_slam.x_hat[1][0])) #self.transformation((self.ekf_slam.x_hat[0][0],self.ekf_slam.x_hat[1][0],self.ekf_slam.x_hat[2][0]), (self.ekf_slam.x_hat[4][0],self.ekf_slam.x_hat[5][0]))   
                # r, g, b = 0, 0 , 255
                # a = 255  # 알파 값
                # rgb = struct.unpack('I', struct.pack('BBBB', b, g, r, a))[0]
            ...
            if(self.first_loop_detect_leg and self.e1 != 0 and self.e2 != 0 and self.step != 5):
            ...
            if(self.first_loop_detect_leg and self.e1 != 0 and self.e2 != 0 and self.step == 6):
                self.first_loop_detect_leg = False
                init_landmark =  [[self.x1, self.y1], [self.x2, self.y2]]
                self.ekf_slam = EKF_SLAM(4, 2, self.Ts, 0 , init_landmark, self.pose_x, self.pose_y, self.yaw)   ## 초기화  self.ekf_slam = EKF_SLAM( 4, 2, self.Ts, 0.05 ,init_landmark) 
                print('초기 1번!!!!',self.ekf_slam.x_hat[4][0], self.ekf_slam.x_hat[5][0])
            ...
            if self.first_loop_detect_leg is not True and self.step == 6:
            ...
            if self.first_loop_detect_leg is not True and self.step == 1:
                ...
                if self.count_wait_between_1_2 > 50:
            ...
            # print(self.e3_hat, 'e3 > distance_center'if (self.e3_hat - self.distance_center) > 0 else 'e3 < distance_center')
    ...
    def lqr_command(self,msg):
        ...
        if (self.step == 2):
            ...
            if (self.count_step2 >= 2):
                self.x1 = (self.x1 + self.count_step2 * self.x1_old) / (self.count_step2 + 1)
                self.y1 = (self.y1 + self.count_step2 * self.y1_old) / (self.count_step2 + 1)
                self.x2 = (self.x2 + self.count_step2 * self.x2_old) / (self.count_step2 + 1)
                self.y2 = (self.y2 + self.count_step2 * self.y2_old) / (self.count_step2 + 1)
        ...
        if(self.step == 5):
            self.count_step5 = self.count_step5 + 1

            if (self.count_step5 >= 2):
                self.x1 = (self.x1 + self.count_step5 * self.x1_old) / (self.count_step5 + 1)
                self.y1 = (self.y1 + self.count_step5 * self.y1_old) / (self.count_step5 + 1)
                self.x2 = (self.x2 + self.count_step5 * self.x2_old) / (self.count_step5 + 1)
                self.y2 = (self.y2 + self.count_step5 * self.y2_old) / (self.count_step5 + 1)
