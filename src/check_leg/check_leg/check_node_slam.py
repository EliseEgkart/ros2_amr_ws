...
class Nodelet2(Node):
    def __init__(self):
        ...
        self.distance_stop_center = 0.31
    ...
    def timer_callback(self):
        ...
        if self.lifecycle == True:
            ...
            if self.step_1_1:
                if self.e3 < 0.5:    # 앞다리 가까워졌을 때 : 두번째 조건
                    self.step_1_2 = True
            ...
            if self.first_loop_detect_leg is not True and self.step == 1:
                ...
                if self.count_wait_between_1_2 > 50:
                    if (abs((self.x1_+self.x2_)/2) < 0.12 and 0.8 <((self.y1_+self.y2_)/2) < 0.92):
                        self.step_3_ready = True
                ...
                if self.count_wait_between_1_2 > 50 and self.step_3_ready:
                    self.step = 3
    ...
    def lqr_command(self,msg):
        ...
        if (self.step == 2):
            ...
            if (abs((self.x1+self.x2)/2) < 0.12 and 0.8 <((self.y1+self.y2)/2) < 0.92):
                ...
                if (self.count_step2 >= 2):
                    ...
                    self.x1_ = (self.x1 + self.count_step2 * self.x1_old) / (self.count_step2 + 1)
                    self.y1_ = (self.y1 + self.count_step2 * self.y1_old) / (self.count_step2 + 1)
                    self.x2_ = (self.x2 + self.count_step2 * self.x2_old) / (self.count_step2 + 1)
                    self.y2_ = (self.y2 + self.count_step2 * self.y2_old) / (self.count_step2 + 1)
