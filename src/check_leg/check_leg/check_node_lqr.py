...
class EKF_SLAM:
    def __init__(self, state_dim, landmark_dim, dt, V, init_landmark):
        self.state_dim = state_dim
        self.landmark_dim = landmark_dim
        self.dt = dt
        self.V = V  # 여기서는 0으로 받음
    ...
    def EKF_propagate(self, x_hat, P_hat, Qe, dt, input):
        x_hat = self.propagate_x(x_hat, dt, input)  # 8*1
    ...
    def propagate_x(self, x, dt, input):
        ...
        xdot[0][0] = x[3][0] * np.cos(x[2][0])  # xdot
        xdot[1][0] = x[3][0] * np.sin(x[2][0])  # ydot
    ...
    def EKF_update(self, x, x_hat, z, P_hat, Re, dt):
        ...
        for j in range(2, (len(x) - 2) // 2 + 1):
            dist3 = np.sqrt((x[2 * j] - x_hat[0]) ** 2 + (x[2 * j + 1] - x_hat[1]) ** 2)
            dist4 = np.sqrt((x_hat[0] - x[2 * j]) ** 2 + (x_hat[1] - x[2 * j + 1]) ** 2)
            

            H3_ = [(x[2 * j + 1][0] - x_hat[1][0]) / (dist3[0] ** 2), -(x[2 * j][0] - x_hat[0][0]) / (dist3[0] ** 2), -1, 0] + \
                [0] * (j - 2) * 2 + [-(x[2 * j + 1][0] - x_hat[1][0]) / (dist3[0] ** 2), (x[2 * j][0] - x_hat[0][0]) / (dist3[0] ** 2)] + \
                [0] * (len(x) - 2 - j * 2)

            H4_ = [(x_hat[0][0] - x[2 * j][0]) / dist4[0], (x_hat[1][0] - x[2 * j + 1][0]) / dist4[0], 0, 0] + \
                [0] * (j - 2) * 2 + [-(x_hat[0][0] - x[2 * j][0]) / dist4[0], -(x_hat[1][0] - x[2 * j + 1][0]) / dist4[0]] + \
                [0] * (len(x) - 2 - j * 2)
    ...
    def generate_z(self, x, x1, y1, x2, y2, landmark_dim):
        ...
        z[2][0] = np.arctan2(y1,x1) - x[2][0]
        z[3][0] = np.sqrt(x1**2 + y1**2)
        z[4][0] = np.arctan2(y2,x2) - x[2][0]
        z[5][0] = np.sqrt(x2**2 + y2**2)
        return z
    
    # def predict(self, control, motion_noise):
    #     # Predict step
    #     x, y, theta = self.state[0:3, 0]
    #     v, w = control

    #     theta_new = theta + self.dt * w
    #     x_new = x + self.dt * v * np.cos(theta_new)
    #     y_new = y + self.dt * v * np.sin(theta_new)

    #     self.state[0:3, 0] = [x_new, y_new, theta_new]



    #     # Motion model Jacobian
    #     G = np.eye(self.state_dim)
    #     G[0, 2] = -v * np.sin(theta_new)
    #     G[1, 2] = v * np.cos(theta_new)

    #     # Control noise Jacobian
    #     V = np.zeros((3, 2))
    #     V[0, 0] = np.cos(theta_new)
    #     V[1, 0] = np.sin(theta_new)
    #     V[2, 1] = 1

    #     # Predict covariance
    #     self.covariance[0:3, 0:3] = G @ self.covariance[0:3, 0:3] @ G.T + V @ motion_noise @ V.T

    # def update(self, measurement, measurement_noise):
    #     z, landmark_id = measurement
    #     landmark_pos = self.state[3 + 2 * landmark_id:5 + 2 * landmark_id, 0]
    #     delta = landmark_pos - self.state[0:2, 0]
    #     q = delta.T @ delta

    #     z_hat = np.array([np.sqrt(q), np.arctan2(delta[1], delta[0]) - self.state[2, 0]])

    #     # Measurement model Jacobian
    #     H = np.zeros((2, self.state_dim))
    #     H[0, 0] = -np.sqrt(q) * delta[0]
    #     H[0, 1] = -np.sqrt(q) * delta[1]
    #     H[0, 3 + 2 * landmark_id] = np.sqrt(q) * delta[0]
    #     H[0, 4 + 2 * landmark_id] = np.sqrt(q) * delta[1]
    #     H[1, 0] = delta[1]
    #     H[1, 1] = -delta[0]
    #     H[1, 2] = -1
    #     H[1, 3 + 2 * landmark_id] = -delta[1]
    #     H[1, 4 + 2 * landmark_id] = delta[0]
    #     H /= q
    ...
    #     z_diff = z - z_hat
    #     z_diff[1] = (z_diff[1] + np.pi) % (2 * np.pi) - np.pi  # Normalize angle
...
class Nodelet2(Node):
    def __init__(self):
        ...
        self.vel_sub = self.create_subscription(WheelMotor, f'/amr{amr_number}/wheelmotor',self.vel_sub_callback, 10)
    ...
    def timer_callback(self):
        ...
        if self.ekf_slam is not None:
            ...
            x = self.ekf_slam.propagate_x(self.ekf_slam.x_hat, self.Ts, self.input)  # true state propagation
            z = self.ekf_slam.generate_z(x, self.x1, self.y1, self.x2, self.y2, self.ekf_slam.landmark_dim)
            self.ekf_slam.x_hat, self.ekf_slam.P_hat = self.ekf_slam.EKF_propagate(self.ekf_slam.x_hat, self.ekf_slam.P_hat, self.ekf_slam.Qe, self.Ts, self.input)  # state propagation
            self.ekf_slam.x_hat, self.ekf_slam.P_hat = self.ekf_slam.EKF_update(x, self.ekf_slam.x_hat, z, self.ekf_slam.P_hat, self.ekf_slam.Re, self.Ts)  # measurement update


            # print(self.ekf_slam.x_hat[4][0],self.ekf_slam.x_hat[5][0])


            print(self.ekf_slam.x_hat[0][0])

            x_,y_ = self.transformation((self.ekf_slam.x_hat[0][0],self.ekf_slam.x_hat[1][0],self.ekf_slam.x_hat[2][0]), (self.ekf_slam.x_hat[4][0],self.ekf_slam.x_hat[5][0]))   
            r, g, b = 0, 0 , 255
            a = 255  # 알파 값
            rgb = struct.unpack('I', struct.pack('BBBB', b, g, r, a))[0]
            clustered_points.append((x_, y_, 0.0, rgb))

            x_,y_ = self.transformation((self.ekf_slam.x_hat[0][0],self.ekf_slam.x_hat[1][0],self.ekf_slam.x_hat[2][0]), (self.ekf_slam.x_hat[6][0],self.ekf_slam.x_hat[7][0]))
            r, g, b = 0, 0 , 255
            a = 255  # 알파 값
            rgb = struct.unpack('I', struct.pack('BBBB', b, g, r, a))[0]
            clustered_points.append((x_, y_, 0.0, rgb))
        ...
        if(self.first_loop_detect_leg and self.e1 != 0 and self.e2 != 0):
            ...
            init_landmark =  [[self.x1, self.y1], [self.x2, self.y2]]
            self.ekf_slam = EKF_SLAM( 4, 2, self.Ts, 0.05 ,init_landmark)   ## 초기화
            print('초기 1번!!!!',self.ekf_slam.x_hat[4][0], self.ekf_slam.x_hat[5][0])
            print('2번!!!!',self.ekf_slam.x_hat[6][0], self.ekf_slam.x_hat[7][0])
