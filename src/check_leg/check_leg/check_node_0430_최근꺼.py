    ...
    def lqr_command(self,msg):
        ...
        if (self.step == 1):
            ...
            if(self.count_step11 > 10): 
                ...
                self.get_logger().info(f'인식 위치 :{(self.x1+self.x2)/2}, {(self.y1+self.y2)/2}')
                if (abs((self.x1+self.x2)/2) < 0.12 and 0.6 <((self.y1+self.y2)/2) < 0.92):
            ...
            if(self.count_step3 > 10): 
                ...
                if (abs((self.x1+self.x2)/2) < 0.12 and 0.6 <((self.y1+self.y2)/2) < 0.95):
                    ...
                    if (self.count_step2 >= 2):
                        ...
                        self.get_logger().info(f'다리 사이 추정 위치 :{(self.x1_+self.x2_)/2}, {(self.y1_+self.y2_)/2}')
